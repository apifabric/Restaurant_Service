...
resourcename="restaurantservice"  # lower case, only
resourcegroup="restaurantservice_rg"
...
# git clone https://github.com/apilogicserver/restaurantservice.git
...
az group create --name restaurantservice_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group restaurantservice_rg --sku S1 --is-linux
az container create --resource-group restaurantservice_rg --name restaurantservice --image apilogicserver/restaurantservice:latest --dns-name-label restaurantservice --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//restaurantservice.westus.azurecontainer.io:5656'
